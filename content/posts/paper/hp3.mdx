  ...
  items={[
    {
      ...
      trigger: "中文译文",
      content:
        "我们搭建了多组仿真实验，以此直观展现现有 RDMA 负载均衡方案存在的缺陷，大家可以结合这两张图表进行查看。图 2 统计了各类方案在不同业务场景下的链路吞吐量不均衡度，图 3 则展示了对应的归一化有效吞吐量表现。从数据中不难发现，ECMP 和 LetFlow 采用静态路由或依赖流间隙做调度，吞吐量不均衡度始终处于高位，网络链路负载差异极大，大量冗余的多路径资源被闲置，整体带宽利用率偏低。反观 RPS、AR 这类细粒度逐包调度方案，虽然能够把流量均匀分发到所有链路上，实现了负载均衡，但频繁的路径切换会造成严重的数据包乱序。RDMA 机制会将报文乱序误判为数据丢包，进而触发大规模无效重传，最终导致系统实际有效吞吐量大幅下滑。综合两组实验结果能够得出结论：传统 RDMA 负载均衡方案都存在明显短板，始终难以在负载均衡、报文保序传输与整体网络性能三者之间做到有效兼顾。",
    },
  ]}
  client:load
/>
